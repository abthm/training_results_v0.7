...
class Bmm2StridedFunction(torch.autograd.Function):
    ...
    def forward(ctx, batch1, mixed, seqlen, batch, maxseqlen, heads, embed, stream, sync, timers):
        ...
        if is_rocm_pytorch:
            mhalib.FastBmm2Fprop(mixed, batch1, output, output, batch, seqlen, heads, embed, False, True, stream, sync)
        else:
    ...
    def backward(ctx, grad_output):
        ...
        if is_rocm_pytorch:
            mhalib.FastBmm2Dgrad1(mixed, grad_output, grad_batch1, grad_batch1, batch, seqlen, heads, embed, False, True, ctx.stream, ctx.sync)
        else:
        ...
        if is_rocm_pytorch:
            mhalib.FastBmm2Dgrad2(grad_output, batch1, grad_mixed, grad_mixed, batch, seqlen, heads, embed, False, True, ctx.stream, ctx.sync)
        else:
