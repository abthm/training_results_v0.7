...
DATA_DIR_PHASE1='/workspace/bert-mlperf-data/mlperf_tf-hdf5'
...
DATA_DIR_PHASE2='/workspace/bert-mlperf-data/mlperf-tf-hdf5'
PREC=""
if [ "$precision" = "fp16" ] ; then
   PREC="--fp16"
elif [ "$precision" = "fp32" ] ; then
...
#	rm -rf rocprof
#	rm rocprofout.*
#	CMD="rocprof --obj-tracking on --timestamp on --hip-trace --roctx-trace -d rocprof -o rocprofout.csv python3.6 $CMD"
#else
#	CMD="python3.6 -m torch.distributed.launch --nproc_per_node=$num_gpus $CMD"
#fi

CMD="python3.6 ./multiproc.py --master_addr="192.168.1.48" --master_port="54321" --node_rank=0 --nnodes=2 --nproc_per_node=$num_gpus $CMD"
