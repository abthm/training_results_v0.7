...
if grep -q "$SUB" <<< "$CMD" ; then
	...
	export RANK=0
	CMD+=" --local_rank=0"
	CMD="nvprof --profile-from-start off --print-gpu-trace --print-api-trace --csv --normalized-time-unit us --log-file nvprof-mlperf-v0.7-8-5-2020-fuse-dropout.csv python3.6 $CMD"
else
